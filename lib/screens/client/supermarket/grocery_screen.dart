...
class _GroceryScreenState extends State<GroceryScreen> {
  ...
  Future<void> _load() async {
    ...
    final s = await SupermarketService().getStoresByCategory('بقالة');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: const Color(0xFF0D0D12), elevation: 0,
        ...
        title: Text('بقالة', style: GoogleFonts.cairo(color: Colors.white, fontWeight: FontWeight.w600)),
        centerTitle: true),
      body: Column(children: [
        Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          child: SmSearchBar(hint: 'ابحث في البقالة...', onChanged: (v) => setState(() => _query = v))),
        Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
