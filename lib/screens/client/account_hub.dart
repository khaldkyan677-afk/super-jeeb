      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            const SizedBox(height: 20),
            _menuSection('حسابي', [
              _menu(Icons.person, 'تعديل الملف الشخصي',
                  () => _open(context, const EditProfileScreen())),
              _menu(Icons.receipt_long, 'طلباتي',
                  () => _open(context, const MyOrdersScreen())),
              _menu(Icons.favorite, 'المفضلة',
                  () => _open(context, const FavoritesScreen())),
              _menu(Icons.local_offer, 'الكوبونات',
                  () => _open(context, const CouponsScreen())),
            ]),
            _menuSection('الإعدادات', [
              _menu(Icons.settings, 'الإعدادات العامة',
                  () => _open(context, const SettingsScreen())),
              _menu(Icons.notifications_active, 'الإشعارات',
                  () => _open(context, const NotificationsScreen())),
              _menu(Icons.security, 'الأمان',
                  () => _open(context, const SecurityScreen())),
              _menu(Icons.devices, 'الأجهزة المتصلة',
                  () => _open(context, const DevicesScreen())),
            ]),
            _menuSection('المساعدة', [
              _menu(Icons.contact_support, 'تواصل معنا',
                  () => _open(context, const HelpScreen())),
              _menu(Icons.info_outline, 'عن التطبيق',
                  () => _open(context, const AboutScreen())),
              _menu(Icons.assignment, 'الشروط والسياسات',
                  () => _open(context, const AboutScreen())),
            ]),
            _menuSection('خطر', [
              _menu(Icons.delete_forever, 'حذف الحساب',
                  () => _confirmDelete(context), color: Colors.red),
            ]),
        ...
        child: Column(
          ...
          children: [
            const CircleAvatar(
              ...
              child:
                  Icon(Icons.person, size: 55, color: Color(0xFF2B2D42)),
            ),
            const SizedBox(height: 12),
            const Text('خالد الأحمدي',
                style: TextStyle(
                    color: Colors.white,
                    fontSize: 18,
            ...
            const Text('client@superjeeb.com',
    ...
    return Container(
      ...
      child: ListTile(
        ...
        leading: Container(
          ...
          child: Icon(icon,
              color: color ?? const Color(0xFFEF233C), size: 20),
        ),
        title: Text(title,
            style: TextStyle(
                fontSize: 13,
...
class _SecurityScreenState extends State<SecurityScreen> {
  ...
  Widget _switchCard(IconData icon, String title, String sub,
